    ...
    if (action.equals("getAuth")) {
      ...
      if (verb.equals("massErase")) {
        ...
        try {
          ...
          Integer[] iarray=new Integer[bytes];
          for (int i = 0; i < barray.length; i++)
            iarray[i]=barray[i] & 0xFF;
          ret.put("start",start);
          ret.put("data",new JSONArray(iarray));
        }  catch (Exception e) {
        ...
        for(Iterator iterator = blocks.keySet().iterator(); iterator.hasNext();) {
          String key = (String) iterator.next();
          Log.e("ARIDD","iterate: "+key);
        }
